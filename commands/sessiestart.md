...
```bash
grep -l "status: actief" $VAULT/02-wiki/*.md 2>/dev/null | wc -l
grep -l "status: concept" $VAULT/02-wiki/*.md 2>/dev/null | wc -l
grep -l "status: stabiel" $VAULT/02-wiki/*.md 2>/dev/null | wc -l
grep -l "status: archief" $VAULT/02-wiki/*.md 2>/dev/null | wc -l
```

## Stap 4: Recente sessies
Laatste 5 sessielogs, alleen filenames:
```bash
ls -1t $VAULT/01-raw/sessies/*.md 2>/dev/null | head -5
```

## Stap 5: Recente wiki-updates
Artikelen gewijzigd in de laatste 7 dagen:
```bash
find $VAULT/02-wiki/ -name "*.md" -mtime -7 -type f 2>/dev/null
```

## Stap 6: Inbox-status
```bash
ls -1 $VAULT/00-inbox/ 2>/dev/null | grep -v '^\.' | wc -l
```
...
```bash
find $VAULT/02-wiki/ -name "*.md" -mtime +60 -type f 2>/dev/null | wc -l
```
